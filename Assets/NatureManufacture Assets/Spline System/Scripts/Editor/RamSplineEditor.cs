...
public class RamSplineEditor : Editor
{
    ...
    void CheckRotations()
    {
        ...
        {
            if (float.IsNaN(spline.controlPointsRotations[i].x) || float.IsNaN(spline.controlPointsRotations[i].y) ||
                float.IsNaN(spline.controlPointsRotations[i].z) || float.IsNaN(spline.controlPointsRotations[i].w))
            {
                spline.controlPointsRotations[i] = Quaternion.identity;
                nan = true;
            }

            if (spline.controlPointsRotations[i].x == 0 && spline.controlPointsRotations[i].y == 0 &&
                spline.controlPointsRotations[i].z == 0 && spline.controlPointsRotations[i].w == 0)
            {
                spline.controlPointsRotations[i] = Quaternion.identity;
                nan = true;
            }
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (spline.transform.eulerAngles.magnitude != 0 || spline.transform.localScale.x != 1 ||
            spline.transform.localScale.y != 1 || spline.transform.localScale.z != 1)
        ...
        else if (spline.toolbarInt == 4)
        {
            ...
            spline.simulatedRiverPoints =
                EditorGUILayout.IntSlider("Simulation points interval", spline.simulatedRiverPoints, 1, 100);
            spline.simulatedMinStepSize =
                EditorGUILayout.Slider("Simulation sampling interval", spline.simulatedMinStepSize, 0.5f, 5);
            ...
            {
                EditorGUI.indentLevel++;
                spline.noiseMultiplierWidth =
                    EditorGUILayout.FloatField("Noise Multiplier Width", spline.noiseMultiplierWidth);
        ...
        else if (spline.toolbarInt == 7)
        {
            ...
            if (spline.toolbarInt == 10)
            {
                ...
                spline.vegetationMaskPerimeter =
 EditorGUILayout.FloatField("Vegetation Mask Perimeter", spline.vegetationMaskPerimeter);
        ...
        if (spline.toolbarInt == 10)
        {
            ...
            spline.vegetationBlendDistance =
 EditorGUILayout.FloatField("Vegetation Blend Distance", spline.vegetationBlendDistance);
    ...
    private void RegenerateVegetationMask()
    {
        ...
        {
            Vector3 position =
 spline.transform.TransformPoint(spline.pointsUp[i]) + (spline.transform.TransformPoint(spline.pointsUp[i]) - spline.transform.TransformPoint(spline.pointsDown[i])).normalized * spline.vegetationMaskPerimeter;
        ...
        for (int i = 0; i < spline.pointsDown.Count; i += 5)
        {
            ...
            Vector3 position =
 spline.transform.TransformPoint(spline.pointsDown[ind]) + (spline.transform.TransformPoint(spline.pointsDown[ind]) - spline.transform.TransformPoint(spline.pointsUp[ind])).normalized * spline.vegetationMaskPerimeter;
            worldspacePointList.Add(position);
        }
    ...
    void RegeneratBiomeMask(bool checkAuto = true)
    {
        ...
        {
            Vector3 position =
 spline.transform.TransformPoint(spline.pointsUp[i]) + (spline.transform.TransformPoint(spline.pointsUp[i]) - spline.transform.TransformPoint(spline.pointsDown[i])).normalized * spline.vegetationMaskSize;
        ...
        for (int i = 0; i < spline.pointsDown.Count; i += (int)(1 / (float)spline.biomMaskResolution))
        {
            int ind = spline.pointsDown.Count - i - 1;
            Vector3 position =
 spline.transform.TransformPoint(spline.pointsDown[ind]) + (spline.transform.TransformPoint(spline.pointsDown[ind]) - spline.transform.TransformPoint(spline.pointsUp[ind])).normalized * spline.vegetationMaskSize;
            worldspacePointList.Add(position);
    ...
    void RiverChannel()
    {
        EditorGUI.BeginChangeCheck();
        EditorGUILayout.Space();
        GUILayout.Label("Single Terrain Select:", EditorStyles.boldLabel);
        EditorGUI.indentLevel++;


        //Debug.Log("terrain " + terrain.name);
        int terrainNumber = spline.terrainsUnder.Count;

        //Debug.Log("splatNumber "+ splatNumber);
        string[] options = new string[terrainNumber];
        for (int i = 0; i < terrainNumber; i++)
        {
            options[i] = i + " - ";

            if (spline.terrainsUnder[i] != null)
            {
                options[i] += spline.terrainsUnder[i].name;
            }
        }


        //Debug.LogError(spline.workTerrain);
        //Debug.LogError("terrainsUnder " + spline.terrainsUnder.Count);
        //Debug.LogError(Terrain.activeTerrain);


        if (spline.terrainsUnder != null && spline.terrainsUnder.Count > 0 &&
            spline.currentWorkTerrain >= spline.terrainsUnder.Count)
        {
            spline.currentWorkTerrain = spline.terrainsUnder.Count - 1;
            spline.workTerrain = spline.terrainsUnder[spline.currentWorkTerrain];
        }
        else if (spline.terrainsUnder == null || spline.terrainsUnder.Count == 0)
        {
            spline.workTerrain = null;
        }
        else
        {
            spline.workTerrain = spline.terrainsUnder[spline.currentWorkTerrain];
        ...
        if (spline.workTerrain == null)
        {
            EditorGUILayout.HelpBox("No terrain under RAM, add terrain and regenerate RAM", MessageType.Warning);
            return;
        }
        ...
        spline.currentWorkTerrain = EditorGUILayout.Popup(spline.currentWorkTerrain, options);
        spline.workTerrain = spline.terrainsUnder[spline.currentWorkTerrain];
        ...
        {
            EditorGUI.indentLevel++;
            spline.noiseMultiplierInside =
                EditorGUILayout.FloatField("Noise Multiplier Inside", spline.noiseMultiplierInside);
            spline.noiseMultiplierOutside =
                EditorGUILayout.FloatField("Noise Multiplier Outside", spline.noiseMultiplierOutside);
        ...
        if (spline.workTerrain && GUILayout.Button("Carve single terrain"))
        {
            spline.ShowTerrainCarve();
            spline.TerrainCarve(spline.workTerrain);
            terrainShapeShow = false;
        }
        ...
        //Debug.Log("terrain " + terrain.name);
        int splatNumber = spline.workTerrain.terrainData.terrainLayers.Length;

        //Debug.Log("splatNumber "+ splatNumber);
        options = new string[splatNumber];
        for (int i = 0; i < splatNumber; i++)
        {
            options[i] = i + " - ";

            //Debug.Log("diffuseTexture is null " + (terrain.terrainData.terrainLayers[i].diffuseTexture == null));
            if (spline.workTerrain.terrainData.terrainLayers[i] != null &&
                spline.workTerrain.terrainData.terrainLayers[i].diffuseTexture != null)
            {
                //Debug.Log(terrain.terrainData.terrainLayers[i].diffuseTexture.name);
                options[i] += spline.workTerrain.terrainData.terrainLayers[i].diffuseTexture.name;
            }
        }
        ...
        {
            EditorGUI.indentLevel++;
            spline.noiseMultiplierInsidePaint =
                EditorGUILayout.FloatField("Noise Multiplier Inside", spline.noiseMultiplierInsidePaint);
            spline.noiseMultiplierOutsidePaint =
                EditorGUILayout.FloatField("Noise Multiplier Outside", spline.noiseMultiplierOutsidePaint);
        ...
        if (splatNumber > 0 && splatNumber > spline.currentSplatMap)
        {
            ...
            if (spline.workTerrain && GUILayout.Button("Paint single Terrain"))
            {
                spline.ShowTerrainCarve();
                spline.TerrainPaintMeshBased(spline.workTerrain);
                terrainShapeShow = false;
            }
        }
        else
        ...
        EditorGUI.indentLevel++;
        spline.distanceClearFoliage =
            EditorGUILayout.FloatField("Remove Details Distance", spline.distanceClearFoliage);
        ...
        spline.distanceClearFoliageTrees =
            EditorGUILayout.FloatField("Remove Trees Distance", spline.distanceClearFoliageTrees);
    ...
    void MeshSettings()
    {
        ...
        if (GUILayout.Button("Create profile from settings"))
        {
            ...
            string path = EditorUtility.SaveFilePanelInProject("Save new spline profile",
                spline.gameObject.name + ".asset", "asset", "Please enter a file name to save the spline profile to");
        ...
        EditorGUILayout.LabelField("Vertice distribution: " + spline.minVal.ToString() + " " +
                                   spline.maxVal.ToString());
    ...
    void DrawVertexColorsUI()
    {
        ...
        if (!spline.showVertexColors)
        {
            if (GUILayout.Button("Show vertex colors"))
            {
                ...
                ResetMaterial();
                spline.GetComponent<MeshRenderer>().sharedMaterial =
    ...
    void DrawFlowColorsUI()
    {
        ...
        if (!spline.showFlowMap)
        {
            if (GUILayout.Button("Show flow directions"))
            {
                ...
                spline.GetComponent<MeshRenderer>().sharedMaterial
                    .SetTexture("_Direction", Resources.Load<Texture2D>("Debug_Arrow"));
        ...
        {
            EditorGUI.indentLevel++;
            spline.noiseMultiplierflowMap =
                EditorGUILayout.FloatField("Noise multiplier inside", spline.noiseMultiplierflowMap);
    ...
    void Tips()
    {
        ...
        EditorGUILayout.HelpBox(
            ...
            MessageType.Info);
        EditorGUILayout.HelpBox(
            "\nKeep resonable quasi- square vertex shapes at river mesh, " +
        ...
        if (spline.beginningSpline != null)
        {
            if (spline.controlPoints.Count > 0 && spline.beginningSpline.points.Count > 0)
            {
                ...
                spline.beginningMaxWidth =
                    Mathf.Clamp(spline.beginningMaxWidth, 0, spline.beginningSpline.vertsInShape - 1);
    ...
    void PointsUI()
    {
        ...
        for (int i = 0; i < spline.controlPoints.Count; i++)
        {
            ...
            if (spline.controlPointsRotations.Count > i)
                spline.controlPointsRotations[i] =
                    Quaternion.Euler(EditorGUILayout.Vector3Field("", spline.controlPointsRotations[i].eulerAngles));
            ...
            if (spline.controlPointsSnap.Count > i)
                spline.controlPointsSnap[i] =
                    EditorGUILayout.Toggle("Snap to terrain", spline.controlPointsSnap[i] == 1 ? true : false) == true
                        ? 1
                        : 0;
            if (spline.controlPointsMeshCurves.Count > i)
                spline.controlPointsMeshCurves[i] =
                    EditorGUILayout.CurveField("Mesh curve", spline.controlPointsMeshCurves[i]);
            EditorGUILayout.Space();
            EditorGUI.indentLevel--;
        }
    }
    ...
    protected virtual void OnSceneGUIInvoke(SceneView sceneView)
    {
        ...
        if (spline != null)
        {
            ...
            for (int j = 0; j < spline.controlPoints.Count; j++)
            {
                ...
                if (screenPoint.z > 0)
                {
                    Handles.Label(handlePos + Vector3.up * HandleUtility.GetHandleSize(handlePos),
                        "Point: " + j.ToString(), style);
                }
                ...
                else if (Tools.current == Tool.Rotate)
                {
                    ...
                    {
                        if (!((spline.beginningSpline && j == 0) ||
                              (spline.endingSpline && j == spline.controlPoints.Count - 1)))
                        {
                            float size = 0.6f;
                            size = HandleUtility.GetHandleSize(handlePos) * size;

                            Handles.color = Handles.zAxisColor;
                            Quaternion rotation = Handles.Disc(spline.controlPointsOrientation[j], handlePos,
                                spline.controlPointsOrientation[j] * new Vector3(0, 0, 1), size, true, 0.1f);
                            ...
                            spline.controlPointsRotations[j] *=
                                (Quaternion.Inverse(spline.controlPointsOrientation[j]) * rotation);

                            if (float.IsNaN(spline.controlPointsRotations[j].x) ||
                                float.IsNaN(spline.controlPointsRotations[j].y) ||
                                float.IsNaN(spline.controlPointsRotations[j].z) ||
                                float.IsNaN(spline.controlPointsRotations[j].w))
                            {
                            ...
                            Handles.CubeHandleCap(0, handlePos, spline.controlPointsOrientation[j], size * 0.3f,
                                EventType.Repaint);

                            Handles.DrawLine(spline.controlPointsUp[j] + spline.transform.position,
                                spline.controlPointsDown[j] + spline.transform.position);
                        }
                    }
                }
            ...
            if (Event.current.type == EventType.MouseUp && Event.current.button == 0 && Event.current.control && !Event.current.alt)
            {
        ...
        if (mesh)
        {
            ...
            for (int i = 0; i < vertices.Length; i++)
            {
                ...
                if (spline.debugFlowmap)
                {
                    ...
                    Handles.Label(vertices[i] + new Vector3(uv4[i].x, uv4[i].y, 0) * 2, uv4[i].x + " " + uv4[i].y,
                        style);
                }
            }
        }
                    ...
                    if (dist < spline.drawSize)
                    {
                        ...
                        else
                        {
                            colorsFlowMap[i] = Vector2.Lerp(colorsFlowMap[i],
                                new Vector2(spline.flowDirection, spline.flowSpeed), spline.opacity * distValue);
                        }
                    }
                }
