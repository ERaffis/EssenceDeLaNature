...
namespace Broccoli.BroccoEditor
{
	...
	public class SproutLabEditor {
		...
		public class StructureSettings {
			...
			public bool displaySnapshotPresetsPopup = false;
		...
		/// <summary>
		/// Popup to display snapshot presets to add.
		/// </summary>
		PopupListComponent snapshotAddPresetPopup;
		/// <summary>
		...
		public SproutLabEditor (VisualElement parentRootVisualElement) {
			...
			InitViewModePopup ();
			snapshotAddPresetPopup = new PopupListComponent (rootVisualElement);
			InitSnapshotAddPresetPopup ();
		...
		public void DrawHeader (Rect windowRect) {
			...
			var enumImpl = _editorImplementations.GetEnumerator ();
			while (enumImpl.MoveNext ()) {
		...
		public void DrawSelectModeView (Rect windowRect) {
			...
			enumImpl = _editorImplementations.GetEnumerator ();
			while (enumImpl.MoveNext ()) {
			...
			switch (currentStructureView) {
				case STRUCTURE_BRANCH: // BRANCHES.
					...
					if (branchNoiseFoldout) {
						EditorGUI.indentLevel++;
						noiseResolution = EditorGUILayout.Slider (labelNoiseResolution, selectedSnapshot.noiseResolution, 0f, 1f);
		...
		bool DrawBranchStructurePanel () {
			...
			for (int i = 0; i <= selectedSnapshot.activeLevels; i++) {
				...
				if (branchFoldouts [i]) {
					EditorGUI.indentLevel++;
					selectedBranchLevelDescriptor = selectedSnapshot.branchLevelDescriptors [i];
		...
		bool DrawSproutAStructurePanel () {
			...
			for (int i = 0; i <= selectedSnapshot.activeLevels; i++) {
				...
				if (sproutAFoldouts [i]) {
					selectedSproutALevelDescriptor = selectedSnapshot.sproutALevelDescriptors [i];
		...
		bool DrawSproutBStructurePanel () {
			...
			for (int i = 0; i <= selectedSnapshot.activeLevels; i++) {
				...
				if (sproutBFoldouts [i]) {
					selectedSproutBLevelDescriptor = selectedSnapshot.sproutBLevelDescriptors [i];
		...
		void DrawSnapshotsPanel () {
			...
			if (GUILayout.Button (addSnapshotGUI)) {
				if (currentStructureSettings.displaySnapshotPresetsPopup) {
					Vector2 mousePos = Event.current.mousePosition;
					mousePos.y += verticalSplitView.GetCurrentSplitOffset ();
					snapshotAddPresetPopup.Open (mousePos);
				} else {
					AddSnapshot ("Main");
				}
			}
		...
		/// </summary>
		void AddSnapshot (string snapshotType, bool clone = false) {
		...
		private void DrawSproutAMapListItemElement (Rect rect, int index, bool isActive, bool isFocused) {
			...
			if (sproutMapArea != null) {
				...
				if (isActive) {
					if (selectedSproutMap != branchDescriptorCollection.sproutStyleA.sproutMapAreas [index]) {
						selectedSproutMap = branchDescriptorCollection.sproutStyleA.sproutMapAreas [index];
		...
		private void DrawSproutBMapListItemElement (Rect rect, int index, bool isActive, bool isFocused) {
			...
			if (sproutMapArea != null) {
				GUI.Label (new Rect (rect.x, rect.y, 150, EditorGUIUtility.singleLineHeight + 5), 
					"Textures for Leaf Type " + (index + 1));
				if (isActive) {
					if (selectedSproutMap != branchDescriptorCollection.sproutStyleB.sproutMapAreas [index]) {
						selectedSproutMap = branchDescriptorCollection.sproutStyleB.sproutMapAreas [index];
		...
		private void DrawCrownMapListItemElement (Rect rect, int index, bool isActive, bool isFocused) {
			...
			if (sproutMapArea != null) {
				GUI.Label (new Rect (rect.x, rect.y, 150, EditorGUIUtility.singleLineHeight + 5), 
					"Textures for Leaf Type " + (index + 1));
				if (isActive) {
					if (selectedSproutMap != branchDescriptorCollection.sproutStyleCrown.sproutMapAreas [index]) {
						selectedSproutMap = branchDescriptorCollection.sproutStyleCrown.sproutMapAreas [index];
		...
		*/
		...
		private void InitSnapshotAddPresetPopup () {
			snapshotAddPresetPopup.width = 200;
			snapshotAddPresetPopup.height = 100;
			snapshotAddPresetPopup.title = "Snapshot Prefabs";
			Dictionary<string, SnapshotSettings> snaps = SnapshotSettings.GetAll ();
			var snapEnum = snaps.GetEnumerator ();
			while (snapEnum.MoveNext ()) {
				Button snapBtn = new Button ();
				snapBtn.text = snapEnum.Current.Value.snapshotType;
				snapBtn.userData = snapEnum.Current.Value.snapshotType;
				string snapshotType = snapEnum.Current.Value.snapshotType;
				snapBtn.clicked += () => {
					AddSnapshot (snapshotType);
					snapshotAddPresetPopup.Close ();
				};
				snapshotAddPresetPopup.listElem.hierarchy.Add (snapBtn);
			}
		}
