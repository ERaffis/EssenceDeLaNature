...
namespace Broccoli.TreeNodeEditor
{
    ...
    public class TextureCanvas : GraphView {
		...
		public void CenterTexture () {
			// Wider texture.
			if (_targetTextureRatio > 1f) {
				float newZoom = guiRect.width / canvasWidth;
				SetPanZoom (new Vector2 (0f, (canvasWidth - canvasHeight) * newZoom * 0.5f), newZoom);
			}
			// Taller texture.
			else {
				float newZoom = guiRect.height / canvasHeight;
				SetPanZoom (new Vector2 ((canvasHeight - canvasWidth) * newZoom * 0.5f, 0f), newZoom);
			}
		}
        #endregion
