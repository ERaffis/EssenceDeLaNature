...
namespace Gaia {
    ...
    public class HydraulicEroder : ITerrainEroder {
        ...
        public void CreateRenderTextures(Vector2Int dim) {
            m_RTSize = dim;
        ...
        public void ReleaseRenderTextures() {
            for (int i = 0; i < 2; i++) {
                if (m_HeightmapRT[i] != null)
                {
                    ...
                    GameObject.DestroyImmediate(m_HeightmapRT[i]); 
                }
                if (m_WaterRT[i] != null)
                {
                    m_WaterRT[i].Release();
                    GameObject.DestroyImmediate(m_WaterRT[i]);
                }
                if (m_WaterVelRT[i] != null)
                {
                    m_WaterVelRT[i].Release();
                    GameObject.DestroyImmediate(m_WaterVelRT[i]);
                }
                if (m_FluxRT[i] != null) 
                {
                    m_FluxRT[i].Release();
                    GameObject.DestroyImmediate(m_FluxRT[i]);
                }
                if (m_SedimentRT[i] != null)
                {
                    m_SedimentRT[i].Release();
                    GameObject.DestroyImmediate(m_SedimentRT[i]);
                }
            }
            if (m_ErodedRT)
            {
                m_ErodedRT.Release();
                GameObject.DestroyImmediate(m_ErodedRT);
            }

            if (m_HardnessRT)
            {
                m_HardnessRT.Release();
                GameObject.DestroyImmediate(m_HardnessRT);
            }
            ...
            if (m_ShowControls) {
                ...
                if (m_ShowAdvancedUI) {
                    ...
                    if (m_ShowThermalUI) {
                        ...
                        m_ErosionSettings.m_ThermalTimeDelta = EditorGUILayout.Slider(ErosionStyles.m_ThermalDTScalar, m_ErosionSettings.m_ThermalTimeDelta, 0.0000f, 0.01f);
                        m_ErosionSettings.m_ThermalIterations = EditorGUILayout.IntSlider(ErosionStyles.m_NumIterations, m_ErosionSettings.m_ThermalIterations, 0, 100);
                        m_ErosionSettings.m_ThermalReposeAngle = EditorGUILayout.IntSlider(ErosionStyles.m_AngleOfRepose, m_ErosionSettings.m_ThermalReposeAngle, 0, 90);
                    }
        ...
        public void OnMaterialInspectorGUI(Terrain terrain, IOnInspectorGUI editContext) {
            ...
            if (m_ShowControls) {
                ...
                m_ErosionSettings.m_MaterialOpacity = EditorGUILayout.Slider(ErosionStyles.m_MaterialOpacity, m_ErosionSettings.m_MaterialOpacity, 0.0f, 1.0f);
                ...
                if (m_ShowAdvancedUI) {
                    ...
                    if (m_ShowThermalUI) {
                        ...
                        m_ErosionSettings.m_ThermalTimeDelta = EditorGUILayout.Slider(ErosionStyles.m_ThermalDTScalar, m_ErosionSettings.m_ThermalTimeDelta, 0.0000f, 0.001f);
                        m_ErosionSettings.m_ThermalIterations = EditorGUILayout.IntSlider(ErosionStyles.m_NumIterations, m_ErosionSettings.m_ThermalIterations, 0, 100);
                        m_ErosionSettings.m_ThermalReposeAngle = EditorGUILayout.IntSlider(ErosionStyles.m_AngleOfRepose, m_ErosionSettings.m_ThermalReposeAngle, 0, 90);
                    }
                    ...
                    m_ShowSedimentUI = EditorGUILayout.Foldout(m_ShowSedimentUI, "Sediment Transport");
                    if (m_ShowSedimentUI) {
                        m_ErosionSettings.m_SedimentScale = EditorGUILayout.Slider(ErosionStyles.m_SedimentScale, m_ErosionSettings.m_SedimentScale, 0.0f, 1.0f);
                        m_ErosionSettings.m_SedimentDepositRate.DrawInspectorGUI();
                        m_ErosionSettings.m_SedimentCapacity.DrawInspectorGUI();
                        m_ErosionSettings.m_SedimentDissolveRate.DrawInspectorGUI();
                    }
        ...
        public void GetShaderParams(ref int pass, ref RenderTexture maskRT) {
            ...
            switch (m_ErosionSettings.m_MaskSourceSelection) {
                ...
                case HydraulicErosionSettings.MaskSource.WaterFlux:
                    ...
                    maskRT = m_FluxRT[1];
                    break;
                case HydraulicErosionSettings.MaskSource.WaterLevel:
                    pass = 0;
                    maskRT = m_WaterRT[1];
                    break;
                case HydraulicErosionSettings.MaskSource.WaterSpeed:
                    pass = 1;
