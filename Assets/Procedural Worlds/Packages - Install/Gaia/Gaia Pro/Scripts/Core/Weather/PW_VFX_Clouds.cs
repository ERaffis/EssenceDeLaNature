            ...
            if (updateDomeBrightness)
            {
                if (CloudLayerMaterials.Count > 0)
                {
                    if (m_gradientOrCurvesExists)
                    {
                        ...
                        {
#if HDPipeline
                            Shader.SetGlobalFloat(GaiaShaderID.m_cloudDomeBrightness, GaiaUtils.SetHDRPFloat(PW_SkyDome_Brightness.Evaluate(PWSkyStandalone.GetTimeOfDayMainValue()), 8f));
#endif
                        }
                        else
                        {
