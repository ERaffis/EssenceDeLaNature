...
namespace Gaia
{
    ...
    public class ProceduralWorldsGlobalWeatherEditor : PWEditor
    {
        ...
        private void OnEnable()
        {
            ...
            if (!Application.isPlaying)
            {
                if (GaiaUtils.CheckIfSceneProfileExists())
                {
                    ...
                    if (sceneProfile.m_selectedLightingProfileValuesIndex <= sceneProfile.m_lightingProfiles.Count - 1)
                    {
                        GaiaLightingProfileValues profile = sceneProfile.m_lightingProfiles[sceneProfile.m_selectedLightingProfileValuesIndex];
                        if (profile != null)
                        {
#if GAIA_2023_PRO
        ...
        private void WindSettingsEnabled(bool helpEnabled)
        {
            ...
            //Wind Speed
            EditorGUILayout.BeginHorizontal();
            m_editorUtils.LabelField("WindSpeed", GUILayout.MaxWidth(EditorGUIUtility.labelWidth - 20));
            EditorGUILayout.LabelField("Calm", GUILayout.MaxWidth(60f));
            m_globalWeather.WindSpeed = EditorGUILayout.Slider(m_globalWeather.WindSpeed, 0f, 1f);
            EditorGUILayout.LabelField("Stormy", GUILayout.MaxWidth(60f));
            EditorGUILayout.EndHorizontal();
            m_editorUtils.InlineHelp("WindSpeed", helpEnabled);
            ...
            if (weatherType == WeatherType.Rain)
            {
                ...
                if (m_globalWeather.EnableRain)
                {
                    ...
                    if (m_globalWeather.m_modifyWind)
                    {
                        ...
                        //Wind Speed
                        EditorGUILayout.BeginHorizontal();
                        m_editorUtils.LabelField("WindSpeed", GUILayout.MaxWidth(EditorGUIUtility.labelWidth - 30));
                        EditorGUILayout.LabelField("Calm", GUILayout.MaxWidth(60f));
                        weatherSettings.m_windSpeed = EditorGUILayout.Slider(weatherSettings.m_windSpeed, 0f, 1f);
                        EditorGUILayout.LabelField("Stormy", GUILayout.MaxWidth(80f));
                        EditorGUILayout.EndHorizontal();
                        m_editorUtils.InlineHelp("WindSpeed", helpEnabled);
                        EditorGUI.indentLevel--;
                    }
