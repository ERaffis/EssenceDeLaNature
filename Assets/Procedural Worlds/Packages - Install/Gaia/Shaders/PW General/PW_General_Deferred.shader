Shader "PWS/PW_General_Deferred"
{
    ...
    SubShader
    {
		...
		//#include "SubGraphs/PW_FloraIncludes.hlsl"
		...
		void vert ( inout appdata_full v, out Input data )
		{
			...
			CoverVertexUV ( worldUVScale, v.vertex, unity_ObjectToWorld, data.coverLayer0UV, data.coverLayer1UV );
			WindCalculations_float ( v.vertex.xyz, _PW_WindTreeWidthHeight, _PW_WindTreeFlex, _PW_WindTreeFrequency, unity_ObjectToWorld, unity_WorldToObject, v.vertex.xyz );

#if defined (_PW_SF_WORLDMAP_ON)
			half2 wmuv;
			WorldMapUV ( worldUVScale, _PW_WorldMapUVScale, unity_ObjectToWorld, wmuv );
