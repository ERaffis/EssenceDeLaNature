...
inline void WindCalculations_float ( in float3 i_vertexPos, in float2 i_widthHeight, in float3 i_flex, in float3 i_frequency,
in float4x4 i_objToWorld, in float4x4 i_worldToObj, out float3 o_vertexPos )
{
	...
	float3 worldOffset = float3(i_objToWorld._m03,i_objToWorld._m13,i_objToWorld._m23) + _WorldSpaceCameraPos;; // grab pos from matrix translation
	...
	if(nrmSqrdDist > 0.0f)
	{
		...
		float3 zScaleRotVec = mul((float3x3)i_objToWorld,up);
		...
		float3 localWorldPos = mul((float3x3)i_objToWorld,i_vertexPos.xyz);
		...
		o_vertexPos = mul((float3x3)i_worldToObj,localWorldPos.xyz);
	}
