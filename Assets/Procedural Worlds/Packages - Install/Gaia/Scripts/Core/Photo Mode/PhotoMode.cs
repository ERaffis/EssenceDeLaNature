    ...
    public class PhotoMode : MonoBehaviour
    {
        ...
        private void Instantiate()
        {
            ...
            if (m_terrainLoader == null)
            {
                ...
                {
                    m_terrainLoader = FindAnyObjectByType<TerrainLoader>();
                }
            }
            ...
            PhotoModeButtonManager buttonManager = FindAnyObjectByType<PhotoModeButtonManager>();
        ...
        private Camera LoadPhotoModeCamera()
        {
            ...
            if (m_spawnedCamera != null)
            {
                ...
                GaiaAPI.SetRuntimePlayerAndCamera(camera.gameObject, camera);
                switch (m_renderPipeline)
                {
                    case GaiaConstants.EnvironmentRenderer.BuiltIn:
                        {
        ...
        private void RemovePhotoModeCamera()
        {
            if (m_targetCamera != null && m_photoModeCameraInstantiated)
            {
                ...
                if (m_lastPlayerController != null)
                {
                    if (m_lastPlayerCamera != null)
                    {
                        GaiaAPI.SetRuntimePlayerAndCamera(m_lastPlayerController, m_lastPlayerCamera);
                    }
                }
                else
                {
                    if (m_lastPlayerCamera != null)
                    {
                        GaiaAPI.SetRuntimePlayerAndCamera(m_lastPlayerCamera.gameObject, m_lastPlayerCamera);
                    }
                }
            }
        }
