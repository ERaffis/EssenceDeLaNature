...
namespace Gaia
{
    ...
    public class ScreenShotter : MonoBehaviour
    {
        ...
        /// </summary>
#if GAIA_INPUT_SYSTEM
        public Key m_screenShotKey = Key.F12;
#endif

        /// <summary>
        /// The storage format, JPG is smaller, PNG is higher quality
        /// </summary>
        public Gaia.GaiaConstants.ImageFileType m_imageFormat = Gaia.GaiaConstants.ImageFileType.Jpg;
        ...
        {
#if GAIA_INPUT_SYSTEM
            if (Keyboard.current[m_screenShotKey].wasPressedThisFrame || m_takeShot)
            {
                TakeScreenshotAndSave();
            }
#else
            //workaround to suppress compilation warning if input system is not used
            if (m_takeShot)
            { 
