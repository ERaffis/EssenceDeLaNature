...
namespace Gaia
{
    ...
    public class UIConfiguration : MonoBehaviour
    {
...
#if GAIA_INPUT_SYSTEM
        public Key m_uiToggleButton = Key.U;
#endif
        ...
        //Puase
#if GAIA_INPUT_SYSTEM
        public Key m_showPauseMenu = Key.Escape;
#endif
        ...
        private bool m_enableUIOnStartup = true;
        #endregion
        ...
        private void Update()
        {
...
#if GAIA_INPUT_SYSTEM
            if (Keyboard.current[m_uiToggleButton].wasPressedThisFrame)
            {
            ...
            if (Keyboard.current[m_showPauseMenu].wasPressedThisFrame)
            {
                if (m_pauseMenu != null)
                {
        ...
        private void Start()
        {
            ...
            if (canvas != null && m_enableUIOnStartup && Application.isPlaying)
            {
                canvas.enabled = true;
            }
        }
        ...
        {
            m_locationManagerText.color = UITextColor;
            m_controlsText.color = UITextColor;
            m_screenShotterText.color = UITextColor;
            m_uiToggleText.color = UITextColor;
        }
        /// <summary>
        /// Updates text size
        /// </summary>
        ...
        {
            m_locationManagerText.fontSize = TextSize;
            m_controlsText.fontSize = TextSize;
            m_screenShotterText.fontSize = TextSize;
            m_uiToggleText.fontSize = TextSize;
        }
        /// <summary>
        ...
        public void ExecuteUIRefresh()
        {
            ...
            if (m_screenShotterText != null)
            {
                //Do we have a screenshotter in the current player setup?
                GameObject runtimeGO = GaiaUtils.GetRuntimeSceneObject(false);
                if (runtimeGO == null)
                {
                    m_screenShotterText.gameObject.SetActive(false);      
                }
                if (runtimeGO.gameObject.GetComponentInChildren<ScreenShotter>() == null)
                {
                    m_screenShotterText.gameObject.SetActive(false);
                }
                else
                {
                    m_screenShotterText.gameObject.SetActive(true);
                }
            }
            ...
            if (m_locationManagerText != null)
            {
                ...
                //Do we have a location manager in the current player setup?
                GameObject playerGO = GaiaUtils.GetPlayerObject(false);
                if (playerGO == null)
                {
                    m_locationManagerText.gameObject.SetActive(false);
                }
                if (playerGO.gameObject.GetComponentInChildren<LocationSystem>() == null)
                {
                    m_locationManagerText.gameObject.SetActive(false);
                }
                else
                {
                    m_locationManagerText.gameObject.SetActive(true);
                    ...
                    if (!string.IsNullOrEmpty(locationProfilePath))
                    {
                        ...
                        if (locationProfile != null)
                        {
                            ...
                            text += string.Format("{0} + {1} (new bookmark), {0} + {2} / {3} (cycle bookmarks)", Keyboard.current[locationProfile.m_mainKey].displayName, Keyboard.current[locationProfile.m_addBookmarkKey].displayName, Keyboard.current[locationProfile.m_prevBookmark].displayName, Keyboard.current[locationProfile.m_nextBookmark].displayName);
            ...
            if (m_freeCamera == null)
            {
                m_freeCamera = FindAnyObjectByType<FreeCamera>();
            }
        }
        ///// <summary>
