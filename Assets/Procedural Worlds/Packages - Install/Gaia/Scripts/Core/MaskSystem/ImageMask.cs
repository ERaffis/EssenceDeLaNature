...
namespace Gaia
{
    ...
    public enum ImageMaskDistanceMaskAxes {[Description("XZ Circular")] XZCircle = 0, [Description("XZ Square")] XZSquare = 3, [Description("X only")] X = 1, [Description("Z Only")] Z =2 }
    ...
    public enum ImageMaskSpace { Local, World }
    ...
    public class ImageMask
    {
        ...
        public float m_xScale = 1f;
        public float m_zScale = 1f;

        public float m_rotationDegrees = 0f;
        public bool m_tiling = false;
        public ImageMaskSpace m_imageMaskSpace = ImageMaskSpace.Local;
        ...
        public ImageMaskDistanceMaskAxes m_distanceMaskAxes = ImageMaskDistanceMaskAxes.XZSquare;
        ...
        public float m_distanceMaskSquareRoundness = 0.25f;
        ...
        /// <summary>
        /// sets the guid for the texture used in the image mask operation directly. Can be used to assign a texture reference without loading the texture. 
        /// If you have a texture loaded already, you can just use imageMask.ImageMaskTexture to assign it.
        /// </summary>
        public void SetTextureGUID(string GUID)
        {
            m_imageMaskTextureGUID = GUID;
        }
        ...
        public RenderTexture Apply(RenderTexture inputTexture, RenderTexture outputTexture)
        {
            ...
            float size = m_multiTerrainOperation.m_isWorldMapPreview ? m_multiTerrainOperation.m_range : m_multiTerrainOperation.m_originTerrain.terrainData.size.x;
            float worldSpaceScaleOffset = m_multiTerrainOperation.m_isWorldMapPreview ? 1f : m_multiTerrainOperation.m_range / m_multiTerrainOperation.m_originTerrain.terrainData.size.x;
            ...
            {
                m_xOffSetScalar = m_xOffSet / size;
                m_zOffSetScalar = m_zOffSet / size;
            }
            ...
            if (m_imageMaskSpace == ImageMaskSpace.World)
            {
                xScale /= worldSpaceScaleOffset;
                zScale /= worldSpaceScaleOffset;
            }

            //size *= inputTexture.width /m_multiTerrainOperation.m_range;
            switch (m_operation)
            {
                case ImageMaskOperation.ImageMask:
                    ...
                    filterMat.SetFloat("_XScale", xScale);
                    filterMat.SetFloat("_ZScale", zScale);
                    filterMat.SetFloat("_Rotation", -1 * (float)((Math.PI / 180) * m_rotationDegrees)); //multiply by -1 to bring it into alignment with the slider on the UI, right on the slider => rotate to the right
                    filterMat.SetInt("_Tiling", m_tiling ? 1 : 0);
                ...
                case ImageMaskOperation.DistanceMask:
                    ...
                    filterMat.SetFloat("_Rotation", -1 * (float)((Math.PI / 180) * m_rotationDegrees)); //multiply by -1 to bring it into alignment with the slider on the UI, right on the slider => rotate to the right
                    filterMat.SetInt("_Tiling", m_tiling ? 1 : 0);
                    filterMat.SetFloat("_SquareRoundness", m_distanceMaskSquareRoundness);
                ...
                case ImageMaskOperation.NoiseMask:
                    ...
                    float previewSize = 1;
                    if (m_multiTerrainOperation.m_originTerrain != null)
                    {
                        previewSize = 1 / m_multiTerrainOperation.m_originTerrain.terrainData.size.x;
                    }
                    else
                    {
                        previewSize = 1 / m_multiTerrainOperation.m_range * 2;
                    }
                    ...
                    #region Alternative noise material setup
                    //Alternative way of setting up the noise material
                    //Material matNoise = new Material(Shader.Find("Hidden/TerrainTools/Noise/NoiseBlit/NoiseBlitFbm"));

                    //int noisePass = 2;
                    //INoiseType noiseType = PerlinNoise.instance;
                    //IFractalType fractalType = FbmFractalType.instance;

                    //switch (m_gaiaNoiseSettings.m_noiseTypeName)
                    //{
                    //    case "Billow":
                    //        noisePass = 0;
                    //        noiseType = BillowNoise.instance;
                    //        break;
                    //    case "Perlin":
                    //        noisePass = 2;
                    //        noiseType = PerlinNoise.instance;
                    //        break;
                    //    case "Ridge":
                    //        noisePass = 4;
                    //        noiseType = RidgeNoise.instance;
                    //        break;
                    //    case "Value":
                    //        noisePass = 6;
                    //        noiseType = ValueNoise.instance;
                    //        break;
                    //    case "Voronoi":
                    //        noisePass = 8;
                    //        noiseType = VoronoiNoise.instance;
                    //        break;
                    //}

                    //// set individual transform info
                    //matNoise.SetVector(ShaderStrings.translation, m_gaiaNoiseSettings.m_translation);
                    //matNoise.SetVector(ShaderStrings.rotation, m_gaiaNoiseSettings.m_rotation);
                    //matNoise.SetVector(ShaderStrings.scale, m_gaiaNoiseSettings.m_scale);

                    //// set full transform matrix
                    //matNoise.SetMatrix(ShaderStrings.transform, m_noiseSettings.trs);

                    //noiseType?.SetupMaterial(matNoise, m_noiseSettings.domainSettings.noiseTypeParams);
                    //fractalType?.SetupMaterial(matNoise, m_noiseSettings.domainSettings.fractalTypeParams);
                    #endregion
                    ...
                    else
                    {
                        ...
                        brushPosWS = (brushPosWS / size) * (size / m_multiTerrainOperation.m_range) * brushSize;
                    }
        ...
        /// <summary>
        /// Sets up the default animation curve for the Base Terrain Generator in the world designer
        /// </summary>
        /// <returns></returns>
        public static AnimationCurve NewAnimCurveBaseTerrainGenerator()
        {
            return new AnimationCurve(new Keyframe[3] { new Keyframe() {
                                                                        inTangent = 1,
                                                                        inWeight = 0,
                                                                        outTangent = 1,
                                                                        outWeight = 0,
