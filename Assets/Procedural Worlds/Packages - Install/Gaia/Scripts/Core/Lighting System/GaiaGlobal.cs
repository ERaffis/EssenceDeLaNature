...
namespace Gaia
{
    ...
    public class GaiaGlobal : MonoBehaviour
    {
        ...
        public static void CheckPostProcessingFog(bool enabled)
        {
            ...
            if (selection != null)
            {
                if (!selection.GetComponent<BiomeController>() && !selection.GetComponent<Spawner>() && !selection.GetComponent<Stamper>() && !selection.GetComponent<GaiaSessionManager>())
                {
                    ...
                    PostProcessLayer layer = GameObject.FindAnyObjectByType<PostProcessLayer>();
                    if (layer != null)
                    {
        ...
        public static void FinalizeCameraObjectRuntime(Camera cameraObject, bool closePhotoMode)
        {
            ...
            CharacterController controller = GameObject.FindAnyObjectByType<CharacterController>();
            ...
            ScreenShotter screenShotter = GameObject.FindAnyObjectByType<ScreenShotter>();
            if (screenShotter != null)
            {
                screenShotter.m_mainCamera = cameraObject;
            }

            FollowPlayerSystem[] followPlayerSystems = GameObject.FindObjectsByType<FollowPlayerSystem>(FindObjectsSortMode.None);
            foreach (var system in followPlayerSystems)
            {
                system.m_player = cameraObject.transform;
            }

            GaiaPlanarReflectionsHDRP reflectionsHDRP = FindAnyObjectByType<GaiaPlanarReflectionsHDRP>();
