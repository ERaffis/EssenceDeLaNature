...
namespace Gaia
{
    ...
    public class GaiaResource
    {
                ...
                case GaiaConstants.SpawnerResourceType.TerrainDetail:
                    ...
                    foreach (DetailPrototype proto in terrain.terrainData.detailPrototypes)
                    {
                        if (detail.m_renderMode == proto.renderMode || detail.m_useInstancing == proto.useInstancing)
                        {
                            ...
                            if (IsSameNatureRendererPrototype(proto, detail))
                            {
                                return localTerrainIdx;
                            }
                        }
            ...
            foreach (ResourceProtoDetail detail in m_detailPrototypes)
            {
                ...
                foreach (DetailPrototype dp in terrainDetails)
                {
                    if (dp.renderMode == detail.m_renderMode)
                    {
                        ...
                        if (IsSameNatureRendererPrototype(dp, detail))
                        {
                            found = true;
                        }
        ...
        private bool IsSameNatureRendererPrototype(DetailPrototype dp, ResourceProtoDetail detail)
        {
                ...
                if (lodGroup != null && dp.prototype != null)
                {
                    if (lodGroup.lodCount > 0)
                    {
                        ...
                        foreach (LOD lod in lodGroup.GetLODs())
                        {
                            ...
                            {
                                MeshFilter mfDetail = lod.renderers[0].GetComponent<MeshFilter>();
                                if (mfDetail != null)
                                {
                                    Renderer otherRenderer = dp.prototype.GetComponent<Renderer>();
                                    if (otherRenderer != null)
                                    {
                                        Material[] materials = lod.renderers[0].sharedMaterials;
                                        Material[] otherMaterials = otherRenderer.sharedMaterials;

                                        if (!materials.SequenceEqual(otherMaterials))
                                        {
                                            continue;
                                        }
                                        ...
                                        if (mfTerrain != null)
                                        {
                                            ...
                                            {
                                                //If we went through all these checks and the meshes are the same, then it muse be the same prototype
                                                foundSame = true;
                                                break;
                                            }
                                        }
