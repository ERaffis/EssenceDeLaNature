...
namespace Gaia.Pipeline.URP
{
    ...
    public static class GaiaURPPipelineUtils
    {
        ...
        private static Light GetSunLight()
        {
            ...
            if (lightObject != null)
            {
                light = Object.FindAnyObjectByType<Light>();
                if (light != null)
                {
...
#endif
#if UNITY_POST_PROCESSING_STACK_V2
                PostProcessLayer postProcessLayer = GameObject.FindAnyObjectByType<PostProcessLayer>();
                if (postProcessLayer != null)
                {
                    GameObject.DestroyImmediate(postProcessLayer);
                }

                PostProcessVolume postProcessVolume = GameObject.FindAnyObjectByType<PostProcessVolume>();
        ...
        public static void CleanUpURP(UnityPipelineProfile profile, GaiaSettings gaiaSettings)
        {
            ...
            {
#if UPPipeline
                UniversalAdditionalCameraData[] camerasData = GameObject.FindObjectsByType<UniversalAdditionalCameraData>(FindObjectsSortMode.None);
                GaiaURPRuntimeUtils.ClearUPCameraData(camerasData);

                UniversalAdditionalLightData[] lightsData = GameObject.FindObjectsByType<UniversalAdditionalLightData>(FindObjectsSortMode.None);
                ...
                TerrainHelper.SetTerrainMaterial(profile.m_builtInTerrainMaterial);
