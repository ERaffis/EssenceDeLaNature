...
namespace Gaia
{
    ...
    public class StamperEditor : PWEditor, IPWEditor
    {
        ...
        private void OnDestroy()
        {
            ...
            //Clean up if stamper was deleted 
            if (m_stamper == null)
            {
...
#region AutoSpawnerList
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (m_stamper.m_seaLevel != SessionManager.GetSeaLevel())
            {
                m_stamper.m_seaLevel = SessionManager.GetSeaLevel();
                m_stamper.m_stampDirty = true;
            }
        ...
        private void DrawStamperControls(bool showHelp)
        {
            ...
            else
            {
                ...
                {
                    if (!SessionManager.CheckIfHeightmapRestoreISAllowed(m_stamper.m_autoSpawners.Where(x => x.isActive == true).Select(x => x.spawner).ToList(), m_gaiaSettings))
                    {
            ...
            {
                if (!SessionManager.CheckIfHeightmapRestoreISAllowed(m_stamper.m_linkedBiomeController.m_autoSpawners.Where(x => x.isActive == true).Select(x => x.spawner).ToList(), m_gaiaSettings))
                {
                    EditorGUIUtility.ExitGUI();
                    return;
                }
        ...
        private void DrawStampSettings(bool showHelp)
        {
            ...
            if (operation == GaiaConstants.FeatureOperation.BlendHeight)
            {
                ...
                m_stamper.m_settings.m_blendStrength = EditorGUILayout.Slider(GetLabel("Blend Strength %"), m_stamper.m_settings.m_blendStrength * 100f, 0f, 100f) /100f;
            }
        ...
        private void DrawInputStampImage(bool showHelp)
        {
            ...
            m_stamper.m_settings.m_stamperInputImageMask.m_imageMaskSpace = (ImageMaskSpace)EditorGUILayout.EnumPopup(m_stamper.m_settings.m_stamperInputImageMask.m_imageMaskSpace, GUILayout.Width(EditorGUIUtility.labelWidth *0.68f));
            GUILayout.Space(10);
            m_editorUtils.LabelField("StampTiling", GUILayout.Width(55));
            m_stamper.m_settings.m_stamperInputImageMask.m_tiling = EditorGUILayout.Toggle(m_stamper.m_settings.m_stamperInputImageMask.m_tiling);
            ...
            m_stamper.m_settings.m_stamperInputImageMask.m_xOffSet = m_editorUtils.FloatField("MaskXOffset", m_stamper.m_settings.m_stamperInputImageMask.m_xOffSet);
            GUILayout.Space(10);
            m_stamper.m_settings.m_stamperInputImageMask.m_zOffSet = m_editorUtils.FloatField("MaskZOffset", m_stamper.m_settings.m_stamperInputImageMask.m_zOffSet);
            ...
            EditorGUIUtility.labelWidth = 10;
            m_stamper.m_settings.m_stamperInputImageMask.m_xScale = m_editorUtils.FloatField("MaskXScale", m_stamper.m_settings.m_stamperInputImageMask.m_xScale);
            GUILayout.Space(10);
            m_stamper.m_settings.m_stamperInputImageMask.m_zScale = m_editorUtils.FloatField("MaskZScale", m_stamper.m_settings.m_stamperInputImageMask.m_zScale);
            EditorGUIUtility.labelWidth = 0;
            GUILayout.EndHorizontal();
            m_editorUtils.InlineHelp("StampScale", showHelp);
            m_stamper.m_settings.m_stamperInputImageMask.m_rotationDegrees = m_editorUtils.Slider("StampRotation", m_stamper.m_settings.m_stamperInputImageMask.m_rotationDegrees, -180f, 180f, showHelp);
        ...
        private void DrawOperation(bool showHelp)
        {
            ...
            if (newSeaLEvel != SessionManager.GetSeaLevel())
            {
        ...
        private void DuringSceneGUI(SceneView obj)
        {
            ...
            m_stamper.DrawStampPreview(null, m_stamper.m_settings.m_isWorldmapStamper);
        }
