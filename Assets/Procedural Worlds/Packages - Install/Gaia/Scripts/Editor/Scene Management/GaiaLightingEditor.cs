        ...
        private void ProfileSettings(bool helpEnabled)
        {
            ...
            if (mode != m_profile.m_lightSystemMode)
            {
                ...
                if (mode == GaiaConstants.GlobalSystemMode.Gaia)
                {
                    ...
                    Gaia.GX.FelixWestin.OverCloudExtension.RemoveOverCloud();
#endif
            ...
            if (m_profile.m_lightSystemMode == GaiaConstants.GlobalSystemMode.Gaia)
            {
                if (m_profile.m_lightingProfiles.Count > 0)
                {
                    ...
                    int[] lightingProfileValuesIndices = Enumerable
                                        .Repeat(0, ((m_profile.m_lightingProfiles.Count - 1) / 1) + 1)
...
#else
                    //only display text box when Gaia pro is not present
                ...
                else
                {
                    ...
                    if (m_renderPipeline != GaiaConstants.EnvironmentRenderer.HighDefinition)
                    {
                        ...
                        if ( m_profileValues.m_ambientMode == AmbientMode.Skybox)
                        {
                            ...
                            if (m_editorUtils.Button("QuickLightBake"))
                            {
...
#if HDPipeline
                        m_editorUtils.Heading("DensityVolumeSettings");
                        EditorGUI.indentLevel++;
                        m_profileValues.m_useDensityVolumeSystem = m_editorUtils.Toggle("UseDensityVolumeSystem", m_profileValues.m_useDensityVolumeSystem, helpEnabled);
                        if (m_profileValues.m_useDensityVolumeSystem)
                        {
                            EditorGUI.BeginChangeCheck();
                            EditorGUI.indentLevel++;
                            m_profileValues.m_densityVolumeResolution = (DensityVolumeResolution)m_editorUtils.EnumPopup("DensityVolumeResolution", m_profileValues.m_densityVolumeResolution, helpEnabled);
                            if (m_profileValues.m_densityVolumeResolution == DensityVolumeResolution.Custom)
                            {
                                EditorGUI.indentLevel++;
                                m_profileValues.m_customDensityVolumeResolution = m_editorUtils.Vector3Field("CustomDensityVolumeResolution", m_profileValues.m_customDensityVolumeResolution, helpEnabled);
                                EditorGUI.indentLevel--;
                            }
                            m_profileValues.m_densityVolumeEffectType = (DensityVolumeEffectType)m_editorUtils.EnumPopup("DensityVolumeEffectType", m_profileValues.m_densityVolumeEffectType, helpEnabled);
                            if (m_profileValues.m_densityVolumeEffectType == DensityVolumeEffectType.Custom)
                            {
                                EditorGUI.indentLevel++;
                                m_profileValues.m_customDensityVolumeEffectType = m_editorUtils.Vector3Field("CustomDensityVolumeEffectType", m_profileValues.m_customDensityVolumeEffectType, helpEnabled);
                                EditorGUI.indentLevel--;
                            }

                            if (EditorGUI.EndChangeCheck())
                            {
                                EditorUtility.SetDirty(HDRPDensityVolumeController.ApplyChanges(m_profileValues));
                            }
